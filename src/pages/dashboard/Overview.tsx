...
const recentActivity = [
  { id: 1, type: 'nomination', user: 'Sarah J.', event: 'Artiste of the Year', time: '2m ago' },
  { id: 2, type: 'follow', user: 'BigWiz Hub', event: null, time: '15m ago' },
  { id: 3, type: 'vote', user: 'Anonymous', event: 'Best Collaboration', time: '22m ago' },
  { id: 4, type: 'payment', user: 'Org Upgrade', event: 'Starter to Pro', time: '1h ago' },
];
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8">
        <div>
           ...
           <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
             <h1 className="text-4xl font-serif text-white tracking-tight mb-2 italic">Welcome, Headies Official</h1>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto">
          <Button variant="outline" className="flex-1 lg:flex-initial h-12 border-white/10" onClick={() => navigate('/onboarding')}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          { title: 'Total Followers', value: '154,200', change: '+12.5%', icon: Users, color: 'text-blue-400' },
          { title: 'Active Votes', value: '2,401,900', change: '+24.1%', icon: Vote, color: 'text-gold-500' },
          { title: 'Page Reach', value: '89.4K', change: '+4.5%', icon: TrendingUp, color: 'text-emerald-400' },
          { title: 'Total Revenue', value: '₦12,450K', change: '+15.2%', icon: DollarSign, color: 'text-white' },
        ].map((stat, i) => (
          <motion.div 
            key={i} 
          ...
          >
            <Card className="hover:border-gold-500/30 transition-all group overflow-hidden relative" onClick={() => navigate('/dashboard/analytics')}>
              ...
              <CardContent className="p-0">
                ...
                <div className="flex items-end justify-between">
                   ...
                   <div className="px-2 py-1 rounded bg-white/5 border border-white/5 text-[10px] font-bold text-emerald-400">
                      {stat.change}
                   </div>
                </div>
              </CardContent>
           ...
           <Card className="bg-dark-900/40 border-white/5">
              <CardHeader className="flex flex-row items-center justify-between">
                 ...
                 <div className="flex gap-2">
                    <button 
                      onClick={() => setChartFilter('Growth')}
                      className={`px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-all ${chartFilter === 'Growth' ? 'bg-gold-500 text-dark-950 shadow-lg shadow-gold-500/20' : 'bg-white/5 border border-white/5 text-dark-400 hover:text-white'}`}
                    >
                      Growth
                    </button>
                    <button 
                      onClick={() => setChartFilter('Reach')}
                      className={`px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-all ${chartFilter === 'Reach' ? 'bg-gold-500 text-dark-950 shadow-lg shadow-gold-500/20' : 'bg-white/5 border border-white/5 text-dark-400 hover:text-white'}`}
                    >
                      Reach
                    </button>
                 </div>
                 ...
                 <div className="h-[340px] w-full">
                    <ResponsiveContainer width="100%" height="100%">
                      <AreaChart data={engagementData} margin={{ top: 10, right: 0, left: -20, bottom: 0 }}>
                        ...
                        />
                        <Tooltip 
                          contentStyle={{ backgroundColor: '#0a0a0a', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '12px', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}
                          itemStyle={{ color: '#fff', fontSize: '12px' }}
                          cursor={{ stroke: 'rgba(198, 138, 53, 0.2)', strokeWidth: 20 }}
                        />
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <Card className="bg-gold-500/5 border-gold-500/10">
                 ...
                 <CardContent className="space-y-4">
                    <textarea 
                       className="w-full bg-dark-950/50 border border-white/5 rounded-xl p-4 text-sm resize-none focus:border-gold-500/50 focus:outline-none h-24"
              ...
              <Card className="bg-dark-900/40">
                 <CardHeader>
                    <div className="flex items-center justify-between">
                       ...
                       <span className="flex items-center px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-500 text-[8px] font-bold uppercase tracking-widest border border-emerald-500/20">
                          Live
                       </span>
                    </div>
                 </CardHeader>
                    ...
                    <div className="flex items-center gap-4">
                       ...
                       <div className="flex-1">
                          <div className="flex justify-between text-[10px] uppercase font-bold tracking-widest mb-1.5">
                             <span className="text-dark-400">Nominee Reach</span>
                             <span className="text-gold-500">82%</span>
                          </div>
                          <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                             <div className="h-full bg-gold-500 w-[82%]" />
                          </div>
                       </div>
                    </div>
                    <div className="flex items-center gap-4">
                       <div className="h-8 w-8 rounded-lg bg-white/5 flex items-center justify-center">
                          <DollarSign className="h-4 w-4 text-dark-500" />
                       </div>
                       <div className="flex-1">
                          <div className="flex justify-between text-[10px] uppercase font-bold tracking-widest mb-1.5">
        ...
        <div className="space-y-8">
           <Card className="border-gold-500/20 relative overflow-hidden">
              ...
              <CardHeader>
                 ...
                 <CardDescription>Current most active portal</CardDescription>
              </CardHeader>
              <CardContent>
                 <div className="aspect-[4/3] rounded-2xl overflow-hidden relative mb-6 group cursor-pointer border border-white/5" onClick={() => navigate('/dashboard/events')}>
                    <img 
                       ...
                       className="absolute inset-0 w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 h-full"
                       alt="Event"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-dark-950 via-dark-950/20 to-transparent" />
                    <div className="absolute bottom-4 left-4">
                       <h4 className="text-white font-serif text-xl mb-1 italic">The Headies 2026</h4>
                       <span className="text-[10px] font-bold text-gold-500 uppercase tracking-widest">Global Broadcast Live</span>
                    </div>
                 </div>
                 <div className="grid grid-cols-2 gap-4 mb-6">
                    <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                       <p className="text-[8px] font-bold text-dark-500 uppercase tracking-widest mb-1">Total Votes</p>
                       <p className="text-white font-medium">1.4M</p>
                    </div>
                    <div className="p-3 rounded-xl bg-white/5 border border-white/5">
                       <p className="text-[8px] font-bold text-dark-500 uppercase tracking-widest mb-1">Duration</p>
                       <p className="text-white font-medium">12d 4h</p>
                    </div>
                 </div>
                 <Button className="w-full h-12 shadow-xl shadow-gold-500/10" onClick={() => navigate('/dashboard/events')}>Manage Broadcast Hub</Button>
              </CardContent>
           </Card>
